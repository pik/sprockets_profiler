(function() {
  ...
  function flameGraph(opts={}) {
    ...
    var labelFormat = function(d) {
      return d[nameKey] + " (" + d3.round(100 * d.dx, 3) + "%, " + d[valueKey] + " samples)";
    };
    ...
    var colorMapper = function(d) {
    	return d.highlight ? "#E600E6" : colorHash(d[nameKey]);
    };
    ...
    function augment(data) {
      ...
      if (data[childrenKey] && (data[childrenKey].length > 0)) {
        data[childrenKey].forEach(augment);
        var childValues = 0;
        data[childrenKey].forEach(function(child) {
          childValues += child[valueKey];
        });
        if (childValues < data[valueKey]) {
          data[childrenKey].push(
            {
              "name": "",
              "value": data[valueKey] - childValues,
              "dummy": true
            }
          );
        }
    ...
    function hide(d) {
      if(!d.original) {
        d.original = d[valueKey];
      }
      d[valueKey] = 0;
      if(d[childrenKey]) {
        d[childrenKey].forEach(hide);
      }
    }
    ...
    function show(d) {
      d.fade = false;
      if(d.original) {
        d[valueKey] = d.original;
      }
      if(d[childrenKey]) {
        d[childrenKey].forEach(show);
      }
    }
    ...
    function getSiblings(d) {
      var siblings = [];
      if (d.parent) {
        var me = d.parent[childrenKey].indexOf(d);
        siblings = d.parent[childrenKey].slice(0);
    ...
    function searchTree(d, term) {
      ...
      function searchInner(d) {
        ...
        if (d[childrenKey]) {
          d[childrenKey].forEach(function (child) {
            searchInner(child);
          });
        }
    ...
    var partition = d3.layout.partition()
      .sort(doSort)
      [valueKey](function(d) {return d.v || d[valueKey];})
      [childrenKey](function(d) {return d.c || d[childrenKey];});
    ...
    function update() {
      ...
      selection.each(function(data) {
        ...
        g.attr("width", function(d) { return d.dx * kx; })
          ...
          .attr("name", function(d) { return d[nameKey]; })
    ...
    function merge(data, samples) {
      samples.forEach(function (sample) {
        ...
        if (node) {
          node[valueKey] += sample[valueKey];
          if (sample[childrenKey]) {
            if (!node[childrenKey]) {
              node[childrenKey] = [];
            }
            merge(node[childrenKey], sample[childrenKey])
          }
        } else {
          data.push(sample);
        }
